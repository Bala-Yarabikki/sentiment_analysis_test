...
def main(_):
    ...
    test_data_loader = torch.utils.data.DataLoader(
        test_dataset,
        batch_size=config.VALID_BATCH_SIZE, num_workers=3)
    ...
    if FLAGS.features:
        ...
        X["texts"] = x_texts
        # X["texts"] = ["@G" + label_decoder(output) if output == value else "@R-" + label_decoder(value) + "-" +
        # label_decoder(output) for output, value in zip(outputs, df_test.label.values)]
